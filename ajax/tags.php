...
if ($_SESSION['logged_in']){
	...
	if (isset($_POST['action']) && isset($_POST['clipid']) && isset($_POST['tagtext'])) {
		...
		// check if clip is taggable - that the clip is either published or the uploader is the owner
		$db->rawQuery('SELECT id FROM clips WHERE (published=1 OR uploader=?) AND id=?',array($userid,$clipid));
		if ($db->count == 1){
			...
			if ($action == 'add') {
				...
				$taglookup = $db->rawQuery('SELECT id FROM tags WHERE tagname=? AND deleted!=1',array($tagtext));
				if ($db->count == 1) {
				...
				if ($clips_x_tags_id) {
					echo json_encode(array('tagsuccess' => true, 'message' => 'Tag "'.$tagtext.'" added to clip '.$clipid.'.'));
				} else {
					exit(json_encode(array('tagsuccess' => false, 'message' => 'Tag "'.$tagtext.'" could not be added to clip '.$clipid.'.')));
				}				
				...
				if ($db->count == 1) {
					...
					$db->rawQuery('DELETE FROM clips_x_tags WHERE clipid=? AND tagid=?',array($clipid,$tagid));

					// check that it completed and return info to the user
					if ($db->count > 0){
						echo json_encode(array('tagsuccess' => true, 'message' => 'Tag "'.$tagtext.'" removed from clip '.$clipid.' '.$db->count.' times.'));
					} else {
						exit(json_encode(array('tagsuccess' => false, 'message' => 'Tag "'.$tagtext.'" could not be removed from '.$clipid.'. Possibly already removed by another user.')));
					}
				} else {
					// trying to delete a tag that does not exist
					exit(json_encode(array('tagsuccess' => false, 'message' => 'This tag does not exist, so it cannot be deleted.')));
				}
			} else {
			...
			// clip not taggable
			exit(json_encode(array('tagsuccess' => false, 'message' => 'Current user is not allowed to tag this clip.')));
		}
	} else {
